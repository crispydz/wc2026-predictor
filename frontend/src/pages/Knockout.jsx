...
function matchTopY(round, idx) { return matchCenterY(round, idx) - MH / 2 }
...
// ── R32 pairs définis ICI (pas dans le composant, pas dans QUADRANTS) ──
const R32_PAIRS = [
  { m:'M74', posA:'1E',  posB:'3(ABCDF)' },
  { m:'M77', posA:'1I',  posB:'3(CDFGH)' },
  { m:'M73', posA:'2A',  posB:'2B'       },
  { m:'M75', posA:'1F',  posB:'2C'       },
  { m:'M76', posA:'1C',  posB:'2F'       },
  { m:'M79', posA:'1A',  posB:'3(CEFHI)' },
  { m:'M78', posA:'2E',  posB:'2I'       },
  { m:'M80', posA:'1L',  posB:'3(EHIJK)' },
  { m:'M83', posA:'2K',  posB:'2L'       },
  { m:'M84', posA:'1H',  posB:'2J'       },
  { m:'M81', posA:'1D',  posB:'3(BEFIJ)' },
  { m:'M82', posA:'1G',  posB:'3(AEHIJ)' },
  { m:'M85', posA:'1K',  posB:'3(DEIJL)' },
  { m:'M86', posA:'1J',  posB:'2H'       },
  { m:'M87', posA:'1B',  posB:'3(EFGIJ)' },
  { m:'M88', posA:'2D',  posB:'2G'       },
]
...
function buildKnockout(bracket, bestThirds) {
  ...
  const r16W = Array.from({length:8}, (_,j) =>
    pickWinner(r32W[j*2], r32W[j*2+1])
  )
  const qfW = Array.from({length:4}, (_,k) =>
    pickWinner(r16W[k*2], r16W[k*2+1])
  )
  ...
  const allRounds = [
    r32,
    Array.from({length:8}, (_,j) => ({ teamA:r32W[j*2], teamB:r32W[j*2+1] })),
    Array.from({length:4}, (_,k) => ({ teamA:r16W[k*2], teamB:r16W[k*2+1] })),
    [{ teamA:qfW[0], teamB:qfW[1] }, { teamA:qfW[2], teamB:qfW[3] }],
    [{ teamA:sf1,    teamB:sf2    }],
  ]

  return { allRounds, r32, r32W, r16W, qfW, sfW:[sf1,sf2], champion }
}
          ...
          <React.Fragment key={i}>
            <div style={{
              ...
              {team
                ...
                : <div style={{width:19,height:13,borderRadius:2,background:'var(--surface4)',flexShrink:0}}/>
              }
              <span style={{
            ...
            {i === 0 && <div style={{height:1,background:'rgba(255,255,255,0.07)'}}/>}
          </React.Fragment>
        )
      })}
    </div>
...
function BracketLines() {
  ...
  for (let r = 0; r < 4; r++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      segs.push(<line key={`h-${r}-${i}`} x1={xR} y1={cy} x2={xM} y2={cy} stroke={S} strokeWidth={1.5}/>)
      if (i%2===0 && i+1<n) {
        const cy2   = matchCenterY(r, i+1)
        const cyMid = (cy+cy2)/2
        segs.push(<line key={`v-${r}-${i}`}  x1={xM} y1={cy}   x2={xM}  y2={cy2}  stroke={S} strokeWidth={1.5}/>)
        segs.push(<line key={`hn-${r}-${i}`} x1={xM} y1={cyMid} x2={xNL} y2={cyMid} stroke={S} strokeWidth={1.5}/>)
      }
    }
  }
  ...
  const xFR = colX(4) + CW
  segs.push(<line key="champ" x1={xFR} y1={finalCy} x2={xFR+LW} y2={finalCy} stroke="rgba(245,166,35,0.5)" strokeWidth={2}/>)
...
function ChampionCard({ team }) {
  ...
  return (
    <div style={{
      ...
      borderRadius:12, padding:'10px 10px 12px', textAlign:'center',
    }}>
      <div style={{fontSize:18,marginBottom:4}}>🏆</div>
      <div style={{display:'flex',justifyContent:'center',margin:'4px 0 6px'}}>
        <Flag team={team} size={28}/>
      </div>
      <div style={{fontSize:11,fontWeight:800,color:'var(--text1)',lineHeight:1.2,marginBottom:4}}>
        {team}
      </div>
...
export default function Knockout() {
  ...
  // ✅ Tout dans le useMemo, dans le composant
  const result = useMemo(() => {
    try { return buildKnockout(bracket, bestThirds) }
    catch(e) { console.error('Knockout error:', e); return null }
  }, [])  // eslint-disable-line
  ...
  const { allRounds, r32, r32W, qfW, sfW, champion } = result

  // ✅ QUADRANTS définis ICI dans le composant, après que result existe
  const QUADRANTS = [
    { title:'QF1', color:'#3b82f6', note:'🇫🇷', pairs:r32.slice(0,4),  r16a:r32W[0],  r16b:r32W[1],  r16c:r32W[2],  r16d:r32W[3],  qfWinner:qfW[0] },
    { title:'QF2', color:'#8b5cf6', note:'🏴󠁧󠁢󠁥󠁮󠁧󠁿', pairs:r32.slice(4,8),  r16a:r32W[4],  r16b:r32W[5],  r16c:r32W[6],  r16d:r32W[7],  qfWinner:qfW[1] },
    { title:'QF3', color:'#f97316', note:'🇪🇸', pairs:r32.slice(8,12), r16a:r32W[8],  r16b:r32W[9],  r16c:r32W[10], r16d:r32W[11], qfWinner:qfW[2] },
    { title:'QF4', color:'#ef4444', note:'🇦🇷', pairs:r32.slice(12,16),r16a:r32W[12], r16b:r32W[13], r16c:r32W[14], r16d:r32W[15], qfWinner:qfW[3] },
  ]
  ...
  return (
    <div className="fade-up">
      <div style={{marginBottom:28}}>
        ...
        <p style={{fontSize:14,color:'var(--text2)'}}>{t('ko.sub')}</p>
      </div>
      ...
      {champion && (
        <div style={{
          ...
          <div style={{fontSize:38,marginBottom:6,filter:'drop-shadow(0 0 18px rgba(245,166,35,0.6))'}}>🏆</div>
          <div style={{fontSize:11,color:'var(--text3)',fontWeight:700,letterSpacing:'0.12em',marginBottom:12}}>
            {t('ko.champion')}
          </div>
          <div style={{display:'flex',justifyContent:'center',marginBottom:8}}>
            <Flag team={champion} size={64}/>
          </div>
          <div style={{fontSize:22,fontWeight:900,color:'var(--text1)',marginBottom:4}}>{champion}</div>
          <div style={{fontSize:30,fontWeight:900,color:'var(--gold)',marginBottom:16}}>
            {(winProb(champion)*100).toFixed(1)}%
          </div>
          <div style={{display:'flex',justifyContent:'center',gap:32}}>
            {sfW.map((f,i) => f && (
              <div key={i} style={{textAlign:'center',opacity:f===champion?1:0.65}}>
                <div style={{display:'flex',justifyContent:'center',marginBottom:4}}>
                  <Flag team={f} size={24}/>
                </div>
                <div style={{fontSize:11,color:'var(--text2)',fontWeight:600,marginBottom:2}}>{f}</div>
                <div style={{fontSize:11,fontWeight:700,color:f===champion?'var(--gold)':'var(--text3)'}}>
                  {f===champion?'🏆 Champion':'🥈 Runner-up'}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card" style={{padding:'20px 16px',overflowX:'auto'}}>
        ...
        <div style={{position:'relative',height:HEADER,minWidth:TOTAL_W,marginBottom:8}}>
          ...
          <div style={{
            position:'absolute',left:colX(4)+CW+LW,width:CHAMP_W,
            textAlign:'center',fontSize:10,fontWeight:800,color:'#f59e0b',letterSpacing:'0.07em',
          }}>CHAMPION</div>
        </div>
        ...
        <div style={{position:'relative',height:TOTAL_H,minWidth:TOTAL_W}}>
          ...
          {[0,1,2,3].map(qi => (
            <div key={qi} style={{
              position:'absolute',left:0,top:qi*SLOT*4,height:SLOT*4,
              width:4,borderRadius:99,
              background:['#3b82f6','#8b5cf6','#f97316','#ef4444'][qi],
              opacity:0.35,
            }}/>
          ))}
      ...
      <div className="card" style={{padding:'24px',marginTop:20}}>
        ...
        <div style={{overflowX:'auto'}}>
          <table className="wc-table">
            <thead>
              <tr>
                {['#','Team','R32','R16','QF','SF','Final','🏆'].map(h=>(
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>

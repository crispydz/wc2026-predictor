...
export default function MatchCard({ match }) {
  ...
  const { team_a, team_b, home_win_prob, draw_prob, away_win_prob,
          expected_goals_a, expected_goals_b, score_distribution } = match
  ...
  const maxP = dist.length > 0 ? dist[0].p : 0.15
  ...
  return (
    <div style={{
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:'var(--text3)',marginTop:2}}>xG {expected_goals_a}</div>
          </div>
        </div>
        <div style={{textAlign:'center',padding:'6px 14px',
          ...
          <div style={{fontSize:10,color:'var(--text3)',fontWeight:600,
                       letterSpacing:'0.05em',marginBottom:3}}>{t('mc.xg')}</div>
          <div style={{fontSize:17,fontWeight:900,color:'var(--gold)',fontFamily:'monospace'}}>
        ...
        <div style={{flex:1,display:'flex',alignItems:'center',gap:8,justifyContent:'flex-end'}}>
          <div style={{textAlign:'right'}}>
            ...
            <div style={{fontSize:11,color:'var(--text3)',marginTop:2}}>xG {expected_goals_b}</div>
          </div>
      ...
      {dist.length > 0 && (
        <div style={{background:'var(--surface2)',borderRadius:10,padding:'10px 12px',
          ...
          <div style={{fontSize:10,color:'var(--text3)',fontWeight:700,
                       letterSpacing:'0.07em',marginBottom:8}}>{t('mc.scores')}</div>
          <div style={{display:'flex',flexDirection:'column',gap:5}}>
            {dist.slice(0,5).map((s,i)=>{
              const outcome = s.ga>s.gb?'home':s.ga<s.gb?'away':'draw'
              const col     = outcome==='home'?'#3b82f6':outcome==='away'?'#ef4444':'#64748b'
              const label   = outcome==='home'?team_a:outcome==='away'?team_b:t('mc.draw')
                ...
                <div key={s.score} style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <div style={{flex:1,height:6,borderRadius:99,
                    ...
                    <div style={{
                      ...
                      background:i===0?`linear-gradient(90deg,${col}99,${col})`:`${col}66`,
                      transition:'width 0.7s ease',
                    }}/>
                  </div>
                  <div style={{fontSize:11,fontWeight:700,color:i===0?'var(--gold)':col,
                               width:38,textAlign:'right',flexShrink:0}}>{pct}%</div>
                  <div style={{fontSize:10,color:'var(--text3)',width:70,
                               overflow:'hidden',textOverflow:'ellipsis',
                               whiteSpace:'nowrap',flexShrink:0}}>{label}</div>
                </div>
              )
            })}
          </div>
      ...
      {/* Win/draw/loss bar */}
      <div>
        <div style={{display:'flex',height:7,borderRadius:99,overflow:'hidden',gap:1.5,marginBottom:6}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',fontSize:12}}>
          <span style={{fontWeight:800,color:'#63B3ED'}}>{team_a.split(' ')[0]} {hw}%</span>
          <span style={{color:'var(--text3)'}}>{t('mc.draw')} {dw}%</span>
          <span style={{fontWeight:800,color:'#FC8181'}}>{aw}% {team_b.split(' ')[0]}</span>
        </div>
      </div>
    </div>
  )
